...
//     // Перемешанные опции для каждого вопроса (в том порядке, в котором пользователь их видел)
//     const shuffledOptions = test.questions.map((q) => shuffleArray(q.options));
  
//     // Правильные индексы в перемешанных вариантах
//     const correctAnswers = test.questions.map((q, i) =>
//       shuffledOptions[i].findIndex((opt) => opt === q.options[q.correctIndex])
//     );
  
//     // Подсчёт баллов
//     let points = 0;
//     userAnswers.forEach((ans, i) => {
//       if (ans === correctAnswers[i]) points++;
//     });
  
//     // Ошибки — индексы неверных ответов
//     const mistakes = userAnswers
//       .map((ans, i) => (ans !== correctAnswers[i] ? i : null))
